...
if(!file_exists(IA_DIR.'install/install.lock') && $_COOKIE['action']!='finish'){
...
if($action == 'env') {
    ...
    foreach ($result['chk_dir'] as $dir) {
        if($dir!='config/db.php'){
            ...
            $a=file_put_contents(IA_DIR .$dir,'Prior to installation');
            // echo $a;die;
            if($a!=21){
                $result['chk_'.md5($dir)] = '<font color=red>[×]不可写</font>';
                $result['iscontinue'] = false;
...
if($action == 'db') {
    if($ispost) {
        ...
        $link = mysql_connect($dbhost.':'.$dbport, $dbuser, $dbpwd);
        ...
        if(file_exists(IA_DIR . 'install/install.sql')){
            $sql = file_get_contents(IA_DIR. '/install/install.sql');
            if(empty($sql)) {
...
//            $dbport = !empty($dbport[1]) ? $dbport[1] : '3306';
            ...
            $config = str_replace(array(
                '{dbhost}', '{dbuser}', '{dbpwd}', '{dbport}', '{dbname}', '{dbtablepre}', '{cookiepre}', '{authkey}'
            ), array(
                "$dbhost", "'$dbuser'", "'$dbpwd'", "$dbport", $dbname, "'$dbprefix'", $cookiepre, $authkey
            ), $config);
            ...
            mysql_query("INSERT INTO {$dbprefix}user (u_name, u_pwd ,u_time) VALUES('{$adminuser}', '{$adminpwd}', '{$u_time}')");

            local_mkdirs(IA_DIR . 'config/');
            file_put_contents(IA_DIR . 'config/db.php', $config);
...
if($action == 'finish') {
    @unlink(IA_DIR . '/install/install.sql');
    tpl_install_finish();
    setcookie('action', '', -10);
...
<div class="header">
	<div class="top">
		<h1 style="color:#ffffff;text-align:center;margin-top:20px;">Young微信管理系统安装程序<sup>　2016</sup></h1>
	</div>
</div>
<div class="main">
	<div class="pleft">
...
function tpl_install_license() {
    echo <<<EOF
		...
		<div class="pr-agreement">
				<p>版权所有 (c)2016，OneTeam微信开发团队保留所有权利。 </p>
				<p>感谢您选择Four groups微9A微信管理系统（以下简称WE，WE基于 PHP + MySQL的技术开发，全部源码开放。</p>
				<p>为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：</p>
			<strong>一、本授权协议适用且仅适用于WE任何版本，WE官方对本授权协议的最终解释权。</strong>
			<strong>二、协议许可的权利 </strong>
				<p>1、您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。 </p>
				<p>2、您可以在协议规定的约束和限制范围内修改 WE 源代码或界面风格以适应您的网站要求。 </p>
			...
			<strong>二、协议规定的约束和限制 </strong>
				...
				<p>4、未经官方许可，禁止在 WE 的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</p>
			...
			<strong>三、有限担保和免责声明 </strong>
				...
				<p>3、电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  WE，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</p>
...
EOF;
    ...
    $insTypes = array();
    //if(file_exists(IA_DIR . '/index.php') && file_exists(IA_DIR . '/setting.php')) {
    //$insTypes['local'] = ' checked="checked"';
    //} else {
    $insTypes['remote'] = ' checked="checked"';
    //}
    $disabled = isset($insTypes['local']) ? ' disabled="disabled"' : '';
    echo <<<EOF
	...
	<table width="726" border="0" align="center" cellpadding="0" cellspacing="0" class="twbox">
		...
		<tr>
			...
			<td>
			  <input name="dbpwd" id="dbpwd" type="password" class="input-txt" />
			</td>
		</tr>
		<tr>
			<td class="onetd"><strong>数据表前缀：</strong></td>
			<td><input name="dbprefix" id="dbprefix" type="text" value="we_" class="input-txt" />
